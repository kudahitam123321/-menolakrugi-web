...
export default function AdminPanel() {
  ...
  async function loadDashboard() {
    ...
    // Leaderboard data
    const { data: membLbData } = await supabase.from('members').select('id,nama,tier,is_advance');
    ...
    setDash({
      ...
      memberProgress: (membLbData||[]).map((m:any)=>({...m,selesai:progByMember[m.id]||0})).sort((a:any,b:any)=>b.selesai-a.selesai),
  ...
  return (
    ...
    <div style={{ fontFamily: C.sans, background: 'linear-gradient(180deg,#050508 0%,#030305 100%)', minHeight: '100vh', color: C.text, display: 'flex', flexDirection: 'column' }}>
      {/* Top bar */}
      <div className='ap-topbar ap-topbar-glow' style={{ borderBottom: `1px solid ${C.border}`, padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(90deg,#07070b,#09090f)', height: 56, flexShrink: 0, position:'sticky' as const, top:0, zIndex:40 }}>
            ...
            <div style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div className='ap-grid2col' style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    {([
                      ...
                      <div key={i} style={{background:'#070710',border:`1px solid ${s.c}22`,padding:'12px',borderRadius:10,position:'relative',overflow:'hidden'}}>
                        ...
                        <div style={{fontFamily:C.mono,color:'#22223a',fontSize:9,marginBottom:6}}>{s.l}</div>
              ...
              <div style={{ background:'linear-gradient(145deg,#0b0b14,#080810)', border:'1px solid #1a1a2a', borderRadius:14, padding:'20px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                  <div style={{ fontFamily:C.mono, color:G.gold, fontSize:9, letterSpacing:2 }}>// LEADERBOARD PROGRESS</div>
                    ...
                    <div key={m.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid #0d0d18'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                          <span style={{fontSize:12,fontWeight:600,color:'#fff',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' as const}}>{m.nama}</span>
                          <span style={{fontFamily:C.mono,fontSize:10,color:G.gold,flexShrink:0,marginLeft:8}}>{m.selesai}/{dash.totalVideos}</span>
                        </div>
                        <div style={{height:3,background:'#111120',borderRadius:2}}>
                          <div style={{height:'100%',width:pct+'%',background:`linear-gradient(90deg,${G.gold},${G.gold}88)`,borderRadius:2}}/>
                        </div>
                      </div>
                    </div>
                  )})
