...
export default function AdminPage({ initialTab, embedded }: { initialTab?: string; embedded?: boolean } = {}) {
  ...
  const [tab, setTab] = useState<'member' | 'video' | 'materi' | 'advance' | 'admins' | 'settings' | 'announce' | 'broker' | 'ulasan' | 'claim' | 'jadwal' | 'rating' | 'referral'>((initialTab as any) || 'member');
    ...
    <div style={{background:'#090909',minHeight:'100%',color:'#e7e5e4',fontFamily:'"Geist",system-ui,sans-serif'}}>
        ...
        {tab === 'rating' && (
          <div style={{display:'flex',flexDirection:'column',gap:12,maxWidth:720}}>
            <div style={{fontFamily:'monospace',color:'#eab308',fontSize:11,letterSpacing:1,marginBottom:4}}>// RATING VIDEO DARI MEMBER</div>
            <div style={{background:'#0d0d0d',border:'1px solid #1f1f1f',borderRadius:8,overflow:'hidden'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 80px 80px 80px',gap:'4px 10px',fontFamily:'monospace',fontSize:9,color:'#444',padding:'10px 16px',borderBottom:'1px solid #111'}}>
                {['VIDEO','LIKES','DISLIKES','SCORE'].map(h=><span key={h}>{h}</span>)}
              </div>
              {videoRatingStats.length===0&&<div style={{padding:'24px',textAlign:'center' as const,fontFamily:'monospace',color:'#333',fontSize:12}}>Belum ada rating.</div>}
              {videoRatingStats.map((v:any,i:number)=>{
                const total = v.likes+v.dislikes||1;
                const pct = Math.round(v.likes/total*100);
                return (
                <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 80px 80px 80px',gap:'4px 10px',alignItems:'center',padding:'10px 16px',borderBottom:'1px solid #0d0d0d'}}>
                  <div>
                    <div style={{fontSize:12,fontWeight:600,marginBottom:4}}>{v.judul}</div>
                    <div style={{height:3,background:'#111',borderRadius:2,overflow:'hidden'}}>
                      <div style={{height:'100%',width:pct+'%',background:'#22ab94',borderRadius:2}}/>
                    </div>
                  </div>
                  <span style={{fontFamily:'monospace',color:'#22ab94',fontWeight:700}}>👍 {v.likes}</span>
                  <span style={{fontFamily:'monospace',color:'#ef4444',fontWeight:700}}>👎 {v.dislikes}</span>
                  <span style={{fontFamily:'monospace',color:pct>=70?'#22ab94':pct>=40?'#eab308':'#ef4444',fontWeight:700}}>{pct}%</span>
                </div>
              )})}
            </div>
          </div>
        ...
        {tab === 'referral' && (
          <div style={{display:'flex',flexDirection:'column',gap:12,maxWidth:720}}>
            <div style={{fontFamily:'monospace',color:'#22ab94',fontSize:11,letterSpacing:1,marginBottom:4}}>// PROGRAM REFERRAL</div>
            <div style={{background:'#0d0d0d',border:'1px solid #1f1f1f',borderRadius:8,overflow:'hidden'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 100px 100px',gap:'4px 10px',fontFamily:'monospace',fontSize:9,color:'#444',padding:'10px 16px',borderBottom:'1px solid #111'}}>
                {['REFERRER','REFERRED','STATUS','AKSI'].map(h=><span key={h}>{h}</span>)}
              </div>
              {adminReferrals.length===0&&<div style={{padding:'24px',textAlign:'center' as const,fontFamily:'monospace',color:'#333',fontSize:12}}>Belum ada referral.</div>}
              {adminReferrals.map((r:any)=>(
                <div key={r.id} style={{display:'grid',gridTemplateColumns:'1fr 1fr 100px 100px',gap:'4px 10px',alignItems:'center',padding:'10px 16px',borderBottom:'1px solid #0d0d0d'}}>
                  <span style={{fontSize:12,fontWeight:600}}>{r.members?.nama||'—'}</span>
                  <span style={{fontSize:12,color:'#888'}}>{r.referred_name||'—'}</span>
                  <span style={{fontFamily:'monospace',fontSize:10,color:r.status==='rewarded'?'#eab308':r.status==='verified'?'#22ab94':'#555',fontWeight:700}}>
                  ...
                  {r.status==='pending'&&(
                    <button onClick={async()=>{
                      ...
                      loadData();
                    }} style={{fontFamily:'monospace',fontSize:9,fontWeight:700,color:'#22ab94',background:'#0a1a14',border:'1px solid #22ab9444',padding:'4px 8px',cursor:'pointer',borderRadius:4}}>
                      VERIFIKASI
                    </button>
                  )}
                  {r.status==='verified'&&(
                    <button onClick={async()=>{
                      ...
                      notify('Reward diberikan!');
                      loadData();
                    }} style={{fontFamily:'monospace',fontSize:9,fontWeight:700,color:'#eab308',background:'#1a1500',border:'1px solid #3a2e00',padding:'4px 8px',cursor:'pointer',borderRadius:4}}>
                      BERI REWARD
                    </button>
                  )}
                  {r.status==='rewarded'&&<span style={{fontFamily:'monospace',fontSize:9,color:'#444'}}>selesai</span>}
                </div>
              ))}
            </div>
          </div>
