...
export function TVTickerTape() {
  React.useEffect(() => {
    // Inject script hanya sekali
    if (document.querySelector('script[src*="tv-ticker-tape"]')) return;
    const script = document.createElement('script');
    script.type = 'module';
    script.src = 'https://widgets.tradingview-widget.com/w/id/tv-ticker-tape.js';
  ...
  return (
    <div style={{ borderBottom: `1px solid ${MR.border}`, overflow: 'hidden' }}>
      {/* @ts-ignore */}
      <tv-ticker-tape
        symbols="FOREXCOM:XAUUSD,FOREXCOM:NAS100,FOREXCOM:US30,FOREXCOM:GBPUSD,FOREXCOM:EURUSD,FOREXCOM:USDJPY,FOREXCOM:GBPJPY,BINANCE:BTCUSDT,FX_IDC:USDIDR"
        item-size="compact"
        color-theme="dark"
      />
    </div>
